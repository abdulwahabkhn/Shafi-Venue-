...
export function AccountsWorkspace(){
 ...
 async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);setError('');const d=new FormData(e.currentTarget);try{await staffApi('users',{id:selected?.id||accountId.current,entry:{name:d.get('name'),username:d.get('username'),role,hall:role==='Hall manager'?d.get('hall'):null,active:d.get('active')==='on',...(d.get('password')?{password:d.get('password')}: {})}});setEditing(false);setMessage('Account saved. Existing sessions for this account were signed out.');await load();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 return <div className="booking-workspace"><div className="ops-page-heading"><div><h1>Staff accounts</h1><p>Set each person’s access. Employee records and login accounts are separate.</p></div><button className="ops-button ops-button--gold" onClick={()=>{accountId.current=crypto.randomUUID();setSelected(null);setRole('Hall manager');setEditing(true);}}>Add account</button></div>{error&&<p className="booking-error" role="alert">{error}</p>}{message&&<p className="booking-success" role="status">{message}</p>}
 {editing&&<section className="ops-panel"><h2>{selected?'Edit account':'New account'}</h2><form key={selected?.id||'new'} onSubmit={save} className="booking-fields"><fieldset disabled={busy}><label>Full name<input required name="name" defaultValue={selected?.name} maxLength={120}/></label><label>Username<input required name="username" pattern="[a-z0-9._-]{3,60}" defaultValue={selected?.username}/></label><label>Role<select value={role} onChange={e=>setRole(e.target.value as Actor['role'])}>{roles.map(r=><option key={r}>{r}</option>)}</select></label>{role==='Hall manager'&&<label>Assigned hall<select name="hall" defaultValue={selected?.hall||'Hall 1'}>{halls.map(h=><option key={h}>{h}</option>)}</select></label>}<label>{selected?'New password (leave empty to keep)':'Password'}<input name="password" type="password" required={!selected} minLength={12} maxLength={128} autoComplete="new-password"/></label><label className="staff-check"><input type="checkbox" name="active" defaultChecked={selected?.active??true}/>Account active</label></fieldset><div className="booking-actions"><button className="ops-button ops-button--gold" disabled={busy}>Save account</button><button type="button" className="ops-button ops-button--quiet" onClick={()=>setEditing(false)}>Cancel</button></div></form></section>}
 <section className="ops-panel"><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Name</th><th>Username</th><th>Role / hall</th><th>Status</th><th>Action</th></tr></thead><tbody>{rows.map(u=><tr key={u.id}><td>{u.name}</td><td>{u.username}</td><td>{u.role}<span>{u.hall}</span></td><td>{u.active?'Active':'Disabled'}</td><td><button className="ops-button ops-button--quiet" onClick={()=>{setSelected(u);setRole(u.role);setEditing(true);}}>Edit</button></td></tr>)}</tbody></table></div>{!rows.length&&<p>No staff accounts yet. Add the GM, accountant and hall managers here.</p>}</section></div>;
}
