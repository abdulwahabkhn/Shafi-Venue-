...
import { roles } from '../../shared/staff';
...
export function AccountsWorkspace(){
 ...
 const [rows,setRows]=useState<StaffUser[]>([]),[editing,setEditing]=useState(false),[selected,setSelected]=useState<StaffUser|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[message,setMessage]=useState('');
 ...
 async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);setError('');try{await staffApi('users',{id:selected?.id||key.current,entry:{name:f.get('name'),username:f.get('username'),role:f.get('role'),hall:null,active:f.get('active')==='on',...(f.get('password')?{password:f.get('password')}:{})}});setEditing(false);setMessage('Account saved. Existing sessions for this account have been signed out.');await load();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 if(actor.role!=='GM')return <p role="alert">Only the GM can manage staff access.</p>;
 return <div className="booking-workspace"><div className="ops-page-heading"><div><h1>Staff accounts</h1><p>GM manages operations. Director monitors. Accountant adds and removes daily expenses.</p></div><button className="ops-button ops-button--gold" disabled={busy||editing} onClick={()=>{key.current=crypto.randomUUID();setSelected(null);setEditing(true);}}>Add account</button></div>{error&&<p role="alert" className="booking-error">{error}</p>}{message&&<p role="status" className="booking-success">{message}</p>}{editing&&<section className="ops-panel"><form onSubmit={save} className="booking-fields" key={key.current}><fieldset disabled={busy}><label>Full name<input name="name" required minLength={2} maxLength={120} defaultValue={selected?.name}/></label><label>Username<input name="username" required pattern="[a-z0-9._-]{3,60}" defaultValue={selected?.username}/></label><label>Role<select name="role" defaultValue={selected?.role||'Accountant'}>{roles.filter(r=>r!=='Hall manager').map(r=><option key={r}>{r}</option>)}</select></label><label>{selected?'New password (leave empty to keep)':'Password'}<input name="password" type="password" required={!selected} minLength={12} maxLength={128} autoComplete="new-password"/></label><label><input type="checkbox" name="active" defaultChecked={selected?.active??true}/>Account active</label></fieldset><div className="booking-actions"><button className="ops-button ops-button--gold" disabled={busy}>Save account</button><button type="button" className="ops-button ops-button--quiet" disabled={busy} onClick={()=>setEditing(false)}>Cancel</button></div></form></section>}<section className="ops-panel"><div className="ops-table-wrap"><table className="ops-table"><thead><tr><th>Name</th><th>Username</th><th>Access</th><th>Status</th><th>Action</th></tr></thead><tbody>{rows.filter(u=>u.role!=='Hall manager').map(u=><tr key={u.id}><td>{u.name}</td><td>{u.username}</td><td>{u.role}</td><td>{u.active?'Active':'Disabled'}</td><td><button className="ops-button ops-button--quiet" disabled={busy||editing} onClick={()=>{key.current=u.id;setSelected(u);setEditing(true);}}>Edit</button></td></tr>)}</tbody></table></div></section></div>;
}
