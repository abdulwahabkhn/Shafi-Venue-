...
try{
 ...
 await deny(store.saveBooking({...base,hall:'Hall 1 + Hall 3'},randomUUID()),'Multi-hall booking rejects a reserved member');
 await store.saveBooking({...base,hall:'Hall 3'},randomUUID());
 await deny(store.saveBooking({...base,hall:'All halls'},randomUUID()),'All halls rejects existing single-hall reservations');
