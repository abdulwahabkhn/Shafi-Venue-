...
for(const [file,key] of [['staff-auth','staffSchema'],['expenses','expenseSchema'],['employees','employeeSchema'],['attachments','attachmentSchema']]){const module=await loader.ssrLoadModule('/server/'+file+'.ts');await store.bookingPool().query(module[key]);}
