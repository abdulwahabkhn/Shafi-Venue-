...
export async function dailyReport(actor:Actor,raw:string|null){
 ...
 const rows=[...booking.rows.map(r=>({id:r.body.id,kind:r.body.kind==='Refund'?'Customer refund':'Booking receipt',person:r.person,amount:r.body.amount,method:r.body.method,status:'Recorded'})),...expense.rows.map(r=>({id:r.body.id,kind:r.body.kind,person:r.body.person,amount:r.body.amount,method:r.body.method,status:r.body.reviewStatus||'Submitted'})),...employee.rows.map(r=>({id:r.body.id,kind:r.body.kind,person:r.body.employeeName,amount:r.body.amount,method:r.body.method,status:'Recorded'}))];
 return {date,rows};
}
