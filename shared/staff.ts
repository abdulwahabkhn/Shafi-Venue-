...
export const accountantPermissionGroups = {Bookings:['booking','bookingReceipt','bookingRefund'],Inventory:['inventoryView','inventoryAdd','inventoryRemove','inventoryDamage','inventoryReplace'],Expenses:['expenseView','expenseAdd','expenseRemove','expenseIssue'],Employees:['employeeView','employeeAdd','employeeRemove','employeeSalary','employeeAdvance'],Attendance:['attendanceView','attendanceEdit'],Reports:['overviewView','reportsView'],Administration:['websiteManage','staffManage']} as const;
export const accountantPermissionKeys = Object.values(accountantPermissionGroups).flat() as unknown as readonly string[];
export type AccountantPermissionKey = 'overviewView'|'websiteManage'|'staffManage'|'booking'|'bookingReceipt'|'bookingRefund'|'inventoryView'|'inventoryAdd'|'inventoryRemove'|'inventoryDamage'|'inventoryReplace'|'expenseView'|'expenseAdd'|'expenseRemove'|'expenseIssue'|'employeeView'|'employeeAdd'|'employeeRemove'|'employeeSalary'|'employeeAdvance'|'attendanceView'|'attendanceEdit'|'reportsView';
...
export function permissionEnabled(actor: Pick<Actor, 'role'|'permissions'>, key: AccountantPermissionKey): boolean {
  ...
  if (role === 'Hall manager') return false;
  if (key === 'staffManage' && role !== 'GM') return false;
  if (key in (actor.permissions || {})) return actor.permissions?.[key] === true;
  return role === 'GM' || (actor.role === 'Accountant' && legacyAccountantPermissions.includes(key));
}
...
export const userInput = z.object({
  ...
  password: z.string().min(8).max(128).optional(), permissions: z.record(z.string(),z.boolean()).refine(value=>Object.keys(value).every(key=>accountantPermissionKeys.includes(key)), 'Unknown permission').default({}),
