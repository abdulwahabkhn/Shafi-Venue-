...
import { expenseCategories, type ExpenseCategory } from './expense-items.js';
import type { FundingSummary, FundingEntry } from './funding.js';
export const monthInput=z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/);
export const expenseRowInput=z.object({name:z.string().trim().min(2).max(160),category:z.enum(expenseCategories).default('Marquee expense'),purpose:z.string().trim().min(2).max(1000),quantity:z.number().int().positive().max(1000000),price:z.number().int().min(0).max(100000000),method:z.enum(['Cash','Bank transfer']).default('Cash')}).refine(v=>v.price*v.quantity<=1000000000,'Total is too large');
export type ExpenseRow={id:string;date:string;name:string;category:ExpenseCategory;purpose:string;quantity:number;price:number;amount:number;method:string;actor:string;removed:boolean};
export type ExpenseSheet={date:string;rows:ExpenseRow[];total:number;cash:number;bank:number;items:string[];categoryTotals:Record<ExpenseCategory,number>;funding:FundingSummary;issues:FundingEntry[]};
