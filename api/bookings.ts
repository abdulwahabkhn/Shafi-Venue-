...
export async function handleBookings(request:RuntimeRequest):Promise<Response>{
 try{
  ...
  if(actor.role==='Accountant'||actor.role==='Hall manager')return send({error:'Booking access is unavailable for this role.'},403);
  if(request.method!=='GET'&&actor.role!=='GM')return send({error:'Only GM can change operational records.'},403);
  ...
  if(data.action==='payment'){requireRole(actor,['GM']);return send(await recordPayment(id,data.payment,actor.name));}
