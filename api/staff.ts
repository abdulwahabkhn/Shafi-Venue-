...
export async function handleStaff(request:RuntimeRequest){try{
 ...
 if(request.method==='GET'){
  ...
  if(resource==='employees'){if(limited&&!hasPermission(actor,'employeeView'))return json({error:'Employee viewing is not enabled for this account.'},403);return json(limited&&!hasPermission(actor,'employeeSalary')&&!hasPermission(actor,'employeeAdvance')?await employeeDirectory(actor):await employeeData(actor));}
 ...
 if(resource==='expense-sheet'){if(limited&&!hasPermission(actor,data.action==='remove'?'expenseRemove':'expenseAdd'))return json({error:'That expense action is not enabled for this account.'},403);return json(data.action==='remove'?await removeExpenseRow(actor,id,data.reason):await addExpenseRow(actor,id,data.entry));}
 if(resource==='inventory-register'){if(limited){const key=data.action==='add'?'inventoryAdd':['Damaged','Write off damage'].includes(data.entry?.action)?'inventoryDamage':['Replaced'].includes(data.entry?.action)?'inventoryReplace':['Remove quantity','Archive'].includes(data.entry?.action)?'inventoryRemove':'inventoryAdd';if(!hasPermission(actor,key))return json({error:'That inventory action is not enabled for this account.'},403);}return json(data.action==='add'?await addInventoryItem(actor,id,data.entry):await inventoryAction(actor,id,data.entry));}
 ...
 if(resource==='employees'){if(limited&&!hasPermission(actor,data.entry?.status&&data.entry.status!=='Active'?'employeeRemove':'employeeAdd'))return json({error:'That employee action is not enabled for this account.'},403);return json(await saveEmployee(actor,id,data.entry,version));}
 ...
 if(resource==='attendance'){if(limited&&!hasPermission(actor,'attendanceEdit'))return json({error:'Attendance editing is not enabled for this account.'},403);return json(await saveAttendance(actor,id,data.entry,version));}
 if(resource==='payment'){if(limited&&!hasPermission(actor,data.entry?.kind==='Advance'||data.entry?.kind==='Advance repayment'?'employeeAdvance':'employeeSalary'))return json({error:'That employee payment action is not enabled for this account.'},403);return json(await recordEmployeePayment(actor,id,data.entry));}
